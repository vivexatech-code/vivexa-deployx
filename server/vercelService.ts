...
export class VercelService {
  ...
  public static async createProject(config: VercelProjectConfig): Promise<{ id: string; name: string }> {
    ...
    if (!res.ok) {
      // If project already exists in Vercel, fetch and update it rather than crashing
  ...
  public static async buildDnsRecords(
    ...
    if (isApex) {
      ...
      // Subdomain (e.g. www or app) requires CNAME
  ...
  public static async addDomainToProject(
    ...
    if (!res.ok) {
      // If domain is already added to this project, fetch its details
      const errMsg = (data.error?.message || '').toLowerCase();
      if (
        res.status === 409 ||
   ...
   * Handles downloading source archive, monorepo root directory resolution, file digest hashing,
   * Vercel v2 file uploading, and v13 deployment creation.
   */
  public static async createDeployment(
    ...
    try {
      // If repoOwner and repoName are provided, download the real repository source code from GitHub
      if (params.repoOwner && params.repoName) {
        fs.mkdirSync(tmpDir, { recursive: true });

        // Retrieve GitHub Token if available
        ...
        // Recursively read all files in source directory (excluding artifacts & large build dirs)
        ...
        // Fallback for manual files provided
      ...
      // Prepare projectSettings for Vercel
      ...
      // Note: Because sourceDir was already adjusted to cleanRootDir,
      // the root directory inside the deployment payload is '.'
      ...
      // Step 1: Submit deployment to Vercel v13 API
      ...
      // Step 2: Handle Vercel missing_files protocol
      if (deployData.error?.code === 'missing_files' && Array.isArray(deployData.error.missing)) {
        ...
        // Upload each missing file to /v2/files
        ...
        // Re-submit the deployment after files are uploaded
