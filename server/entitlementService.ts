...
import { getPlanFromFirebase, FirebasePlan } from './plansData';
...
export class EntitlementService {
   ...
   * Checks status === 'active', verified paymentStatus === 'paid' (or valid razorpayPaymentId),
   * and unexpired billing period.
   */
  public static async getUserActiveSubscription(adminDb: any, userId: string): Promise<any | null> {
    try {
      ...
      // Verify valid planId in Firebase
   ...
   * NEVER defaults to Starter or any paid plan.
   */
   ...
   * Rejects immediately if user has no active subscription.
   */
  ...
  public static async canDeploy(
    ...
    // Count user's deployments created in the past 30 days
    try {
      ...
      // Fallback if query on createdAt needs index
      return {
        allowed: true,
        currentCount: 0,
        maxLimit: plan.maxDeployments,
